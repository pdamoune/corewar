...
/*   Updated: 2017/09/19 21:04:17 by wescande         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			update_players(t_vm *vm, int id)
{
	...
	else if (vm->file[id].live == god_players(vm))
	{
		if (vm->gtk.god_players != -1 && vm->gtk.god_players != id)
			gtk_label_set_text(GTK_LABEL(vm->gtk.panel.players[vm->gtk.god_players].status), STATUS_ALIVE);
	...
	gtk_label_set_text(GTK_LABEL(vm->gtk.panel.players[id].last_live), ft_litoa_nomalloc(vm->file[id].last_live, txt));
	gtk_label_set_text(GTK_LABEL(vm->gtk.panel.players[id].n_live), ft_itoa_nomalloc(vm->file[id].live, txt));
}
