...
/*   Updated: 2017/09/19 20:08:15 by wescande         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static GtkWidget	*create_carry(t_process_win *p_win)
{
	...
	box = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
	gtk_box_pack_start(GTK_BOX(box), gtk_label_new("Carry :"), FALSE, FALSE, 15);
...
GtkWidget			*create_info_win(t_vm *vm)
{
	...
	i = -1;
	while (++i < REG_NUMBER)
		gtk_box_pack_start(GTK_BOX(box), create_one_register(p_win, i), FALSE, FALSE, 15);
